    ...
    def create(self, validated_data):
        ...
        if required_online_judge == "codeforces":
            ...
            if codeforces_username is None:
                raise serializers.ValidationError(detial="Codeforces Handle is Not Available")
        ...
        if solved_status is False:
            raise serializers.ValidationError(detail="Problem has not been solved")

        existing_submission = Submission.objects.filter(
            problem=required_problem, correct_answer=True).first()

        if existing_submission is not None:
            raise serializers.ValidationError(detail="Problem is already solved by the user")
